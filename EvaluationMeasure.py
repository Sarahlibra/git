...
def score(S, T):
	...
	for result in result_file:
		...
		if u in T and len(r)>0:
			t = T[u] #Test itemList
			score1 = precisionAtK(r, t, 2)
			score2 = recall(r,t)
			score3 = userSuccess(r,t)
...
def precisionAtK(recommendedItems, relevantItem, k):
	topK = recommendedItems[0:k-1] #takes first k items from the list of reccommendedItems
	return len(list(set(topK).intersection(set(relevantItem))))/k
...
def recall(recommendedItems, relevantItem):
	if (len(relevantItem)> 0):
		return	len(list(set(recommendedItems[0:29]).intersection(set(relevantItem))))/len(relevantItem)
	else: 
		return 0.0
#user success = was at least one relevant item recommended for a given user?
def userSuccess(recommendedItems, relevantItem):
	if len(list(set(recommendedItems[0:29]).intersection(set(relevantItem))))> 0:
		return 1.0
	else:
		return 0.0
